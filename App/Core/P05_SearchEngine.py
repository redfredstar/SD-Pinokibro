...
class P05_SearchEngine:
    ...
    def __init__(self, data_file_path: Optional[str] = None):
        ...
        if data_file_path is None:
            ...
            data_file_path = os.path.join(current_dir, "..", "Data", "cleaned_pinokio_apps.json")
        ...
        try:
            ...
            for app_data in data:
                try:
                    ...
                    logging.warning(f"Failed to process app entry: {app_data}. Error: {str(e)}")
                    continue
    ...
    def search(self, query: str, filters: Optional[Dict[str, Any]] = None,
               limit: int = 20) -> List[Tuple[PinokioApp, float]]:
        """
            ...
            if tags:
                ...
                for tag in tags[1:]:
                    if tag in self.tag_index:
                        tag_filtered_ids.intersection_update({app.id for app in self.tag_index[tag]})
                    else:
                        return []  # No apps with this tag
                        
                # Convert back to list and intersect with current filtered_apps
                filtered_apps = [app for app in filtered_apps if app.id in tag_filtered_ids]
                
        # GPU requirement filter
        if 'gpu_required' in filters:
            gpu_required = bool(filters['gpu_required'])
            filtered_apps = [app for app in filtered_apps if app.gpu_required == gpu_required]
            
        # Size filter
        if 'max_size_mb' in filters:
            max_size = filters['max_size_mb']
            filtered_apps = [app for app in filtered_apps if app.size_mb and app.size_mb <= max_size]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "avg_size_mb": sum(app.size_mb for app in self.apps 
