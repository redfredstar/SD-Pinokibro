...
class TestEnvironmentManager:
        ...
        try:
            ...
            for attr in required_attrs:
                if not hasattr(platform_info, attr):
                    self.log_test(
                        "Platform Detection - Required Attributes",
                        False,
            ...
            self.log_test(
                "Platform Detection", True, f"Detected: {platform_info.platform_name}"
            )
    ...
    def test_environment_type_detection(self):
        ...
        try:
            ...
            expected_type = (
                "conda"
                if not self.cloud_detector.detect_platform().is_cloud
                else "venv"
            )

            if env_type == expected_type:
                self.log_test(
                    "Environment Type Detection - Default",
                    True,
                    f"Expected: {expected_type}, Got: {env_type}",
                )
            else:
                self.log_test(
                    "Environment Type Detection - Default",
    ...
    def test_conda_environment_creation(self):
        ...
        try:
            ...
            if result.returncode != 0:
                self.log_test(
                    "Conda Environment Creation", False, "Conda not available"
                )
                return
            ...
            if success:
                self.log_test(
                    "Conda Environment Creation", True, f"Created: {test_env_name}"
                )
                ...
                if env_type == "conda":
                    self.log_test(
                        "Conda Environment Detection", True, f"Detected as: {env_type}"
                    )
                else:
                    self.log_test(
                        "Conda Environment Detection",
                        False,
                        f"Expected: conda, Got: {env_type}",
                    )
                ...
                if prefix and "conda" in prefix.lower():
                    ...
                    self.log_test(
                        "Conda Run Prefix", False, f"Invalid prefix: {prefix}"
                    )
    ...
    def test_venv_environment_creation(self):
        ...
        try:
            ...
            self.cloud_detector.detect_platform = lambda: type(
                "PlatformInfo",
                (),
                {
            ...
            if success:
                self.log_test(
                    "Venv Environment Creation", True, f"Created: {test_env_name}"
                )
                ...
                if env_type == "venv":
                    self.log_test(
                        "Venv Environment Detection", True, f"Detected as: {env_type}"
                    )
                else:
                    self.log_test(
                        "Venv Environment Detection",
                        False,
                        f"Expected: venv, Got: {env_type}",
                    )
        ...
        try:
            ...
            try:
                ...
                self.log_test(
                    "Error Handling - Empty Name",
                    True,
                    f"Correctly raised exception: {str(e)}",
                )
