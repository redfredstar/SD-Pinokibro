...
### Code Snippet
```python
def install_app(self, recipe: List[Dict], app_name: str, callback: Callable[[str], None]) -> P07_InstallationResult:
    """
    ...
    try:
        ...
        raise Exception(f"Installation orchestration failed: {str(e)}\n{traceback.format_exc()}")
```
...
def create(self, env_name: str, callback: Optional[Callable[[str], None]] = None) -> int:
    ...
    try:
        ...
        raise Exception(f"Environment creation failed: {str(e)}\n{traceback.format_exc()}")
```
...
```python
def _parse_json(self, json_data: Dict) -> List[RecipeStep]:
    """
...
### Code Snippet
```python
def _execute_single_step(self, step: RecipeStep, run_prefix: str, 
    ...
    try:
        ...
        raise Exception(f"Recipe step execution failed: {str(e)}\n{traceback.format_exc()}")
```
